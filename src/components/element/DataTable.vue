<template>
  <div class="dataTable">
    <div class="headTitle" v-if="toolMenuFlag">
      ...
      <slot name="headButton" v-else> </slot>
      <el-button-group v-if="menuFlag" style="float:right">
        <el-tooltip
          effect="dark"
          :content="showHandleFlag ? 'common.fold' : 'common.open'"
          placement="top"
        >
          <el-button
            :icon="showHandleFlag ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"
            @click="showHandleFlag = !showHandleFlag"
          ></el-button>
        </el-tooltip>
          ...
          <el-dropdown-menu slot="dropdown">
            <template v-for="(column, index) in columns">
              <el-dropdown-item :key="index">
                <el-checkbox v-model="columnMenus[column.prop]">{{
                  column.label
                }}</el-checkbox>
              </el-dropdown-item>
            </template>
    ...
    >
        ...
        :index="
          index => {
            return (
              index +
              1 +
              (queryParas.currentPage - 1) * queryParas.currentPageSize
            )
          }
        "
        align="center"
            ...
            <template v-for="(columnChild, ind) in column.child">
              ...
              >
                <template slot-scope="scope">
                  ...
                  <span>{{
                    columnChild.format
                      ? columnChild.format(scope.row)
                      : scope.row[columnChild.prop]
                  }}</span>
                </template>
              </el-table-column>
            </template>
      ...
      <el-table-column
        v-if="
          operateBtnLayout.indexOf('V') > -1 ||
            operateBtnLayout.indexOf('E') > -1 ||
            operateBtnLayout.indexOf('R') > -1 ||
            operateBtnLayout.indexOf('D') > -1 ||
            operateBtnLayout.indexOf('U') > -1
        "
        fixed="right"
        align="center"
        label="操作"
      ...
      >
        <template slot-scope="scope">
          <operateGroup
            ...
            :viewPerms="viewPerms"
            :editPerms="editPerms"
            :reviewPerms="reviewPerms"
            :uploadPerms="uploadPerms"
            :delPerms="delPerms"
            :deleteApi="deleteApi"
            :autoDel="autoDel"
            :operateColLayout="operateColLayout(scope.row)"
            :rowData="scope.row"
            :rowIndex="scope.$index"
            @operateBtnClick="operateBtnClick"
          >
            <template slot="button" slot-scope="obj">
              <slot name="button" :row="obj.row" :index="obj.index"> </slot>
            </template>
          </operateGroup>
        </template>
      </el-table-column>
...
export default {
  ...
  computed: {
    ...
    getHeight() {
      // 100%时，内容铺满
      if (this.otherHeight === '100%')
        return this.$nextTick(() => {
          '100%'
        })
      // 设置talbe 最小高度
      if (this.$store.getters.contentHeight - this.otherHeight < 150)
        return 150 + 'px'
  ...
  methods: {
    ...
    loadTableData: function() {
      ...
      if (this.showPage) {
        this.vLoading = true
        this.api[this.loadPage](this.queryParas)
          .then(res => {
            if (res.code === 0) {
              if (res.data) {
